...
SKIP_RESP_HEADERS = {
    ...
    "te", "trailers", "upgrade",
    "content-encoding",  # مهم: requests خودش decompress می‌کنه
}
# ─────────────────────────────────────────────────────────────────────────────
...
def do_fetch(item: dict) -> dict:
    ...
    # Accept-Encoding رو ست نمی‌کنیم
    # requests خودش decompress می‌کنه ولی Content-Encoding رو فوروارد می‌کنیم
    # که باعث double decompress میشه — پس بهتره بدون compression بریم
    ...
    try:
        ...
        # Content-Encoding رو حذف می‌کنیم چون requests خودش decompress کرده
    ...
    try:
        ...
        if "q" in req and isinstance(req["q"], list):
            ...
            with ThreadPoolExecutor(max_workers=min(WORKERS, len(items))) as ex:
                futures = {ex.submit(do_fetch, item): i
...
def health():
    ...
    return jsonify({"status": "ok", "cache": cs,
                    "dns_cache": ds, "workers": WORKERS})
